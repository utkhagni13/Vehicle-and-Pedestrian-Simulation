...
VEHICLE_LENGTH = 80
VEHICLE_WIDTH = 30
VEHICLE_MAX_VELOCITY = 10.0
VEHICLE_MAX_ACC = 30.0
VEHICLE_START_Y = SCREEN_HEIGHT // 2 - VEHICLE_WIDTH // 2

PEDESTRIAN_MAX_VELOCITY = 3.0
PEDESTRIAN_DES_VELOCITY = 1.50
PEDESTRIAN_START_Y = SCREEN_HEIGHT // 2 - VEHICLE_WIDTH // 2
...
class Pedestrian(pygame.sprite.Sprite):
    def __init__(self, m, n):
        ...
        x = SCREEN_WIDTH // 2 - self.image.get_rect().width // 2 + m
        y = SCREEN_HEIGHT // 2 - LANE_WIDTH // 2 - 40 + n
        ...
        self.desr_vel = np.array([0, PEDESTRIAN_DES_VELOCITY])
    ...
    def updateMotivation(self, D_pv, cv, ca):
        t_adv = (D_pv/cv) - (LANE_WIDTH/magnitude(self.desr_vel)) - t_reaction
        f = np.array([t_adv, ca])
        # print(t_adv, f)
    ...
    def move(self, car_pos, car_vel, car_acc):
        ...
        # calc. F_speed
        F_speed = np.array([ 0, A_speed * self.sgn(y) * math.exp(-(x - a)/(cv * delT)) * math.exp(-(y**2)/(2 * sigma_y**2)) ])
        ...
        acc = magnitude(self.acceleration)
        if acc > PEDESTRIAN_MAX_ACC:
            self.acceleration = PEDESTRIAN_MAX_ACC/acc * self.acceleration

        # move the pedestrian
        # print(f"vel: {self.curr_vel}, {}")
        # r1 = math.sqrt(abs(self.curr_vel[0]**2 + 2 * (self.goal - self.pos)[0] * self.acceleration[0]))
        # r2 = math.sqrt(abs(self.curr_vel[1]**2 + 2 * (self.goal - self.pos)[1] * self.acceleration[1]))
        self.curr_vel = self.curr_vel + self.acceleration
        vel = magnitude(self.curr_vel)
        # if vel > PEDESTRIAN_MAX_VELOCITY:
        self.curr_vel = PEDESTRIAN_MAX_VELOCITY/vel * self.curr_vel
        ...
        print(f"ped_vel: {self.curr_vel}, ped_pos: {self.pos}, car_pos: {car_pos}, car_vel: {car_vel}")
        
        if self.pos[1] > SCREEN_HEIGHT // 2 + LANE_WIDTH:
            self.curr_vel = 0
            self.acceleration = 0
            self.stop = 1
...
class Car(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.pos = np.array([0, VEHICLE_START_Y])

        self.curr_vel = np.array([VEHICLE_MAX_VELOCITY, 0])
        self.acceleration = np.array([VEHICLE_MAX_ACC, 0])
    ...
    def move(self):
        ...
        if STOP == 1:
            return
        self.curr_vel[0] = VEHICLE_MAX_VELOCITY
